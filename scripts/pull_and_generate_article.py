"""Pull latest messages from Telegram channels, process events, and generate long-read Article."""
...
setup_logging("INFO")
logger = logging.getLogger("pull_and_generate_article")
...
async def main() -> None:
    ...
    # 5. Generate article
    logger.info("5. Generating Event-First Long-Read Article (24h window)...")
    ...
    run = await snapshot_service.create_run(
        ...
        snapshot_at=now,
        request_key=f"cli:article:fresh:{now.isoformat()}",
        config=config,
    ...
    try:
        # Rule: Never deliver to Telegram in tests/CLI generation (use defer_delivery=True)
        ...
        logger.info("Saved article to %s (%d chars)", article_path, len(article_text))
        ...
        print(f"Причина: {exc.reason} ({exc.error_kind})")
        print("═" * 80)
