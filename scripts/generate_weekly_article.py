"""Generate and publish the Event-First Weekly Long-Read (Итоги недели).

Usage:
    python scripts/generate_weekly_article.py [--edition berdyansk] [--date YYYY-MM-DD] [--deliver] [--dry-run]
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate Event-First Weekly Long-Read Article (Итоги недели)."
    )
    parser.add_argument(
        "--edition",
        default="berdyansk",
    ...
    parser.add_argument(
        "--date",
        default=None,
        help="Target date for the weekly report (YYYY-MM-DD, defaults to current time)",
    )
    parser.add_argument(
        "--deliver",
        action="store_true",
...
async def main() -> None:
    ...
    if args.date:
        ...
        snapshot_at = dt.datetime(
            parsed_date.year, parsed_date.month, parsed_date.day, 19, 0, tzinfo=tz
        )
    else:
    ...
    req_key = f"cli:weekly_article:{edition_slug}:{snapshot_at.date().isoformat()}"
    if not args.dry_run:
        try:
            ...
            print(
                f"Обновляю источники и ставлю еженедельную публикацию в очередь "
    ...
    print(f"\n🚀 Запуск генерации еженедельного лонгрида для издания '{edition.name}'...")
    print(f"📅 Окно среза: {snapshot_at.isoformat()} (168 часов lookback)")
    ...
    try:
        ...
        if args.deliver and not args.dry_run:
            ...
            print("🎉 Еженедельный лонгрид успешно опубликован!")
        elif args.dry_run:
