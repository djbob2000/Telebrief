...
7. **Publication Candidate Eligibility, Digest Rollup & Scan-First UX** (`src/publication/repository.py`, `src/publication/city_situation.py`, `src/publication/renderers.py`, `src/publication/digest_narrative.py`):
   ...
   - Synthesizes scan-first `DigestEditorialItemDraft` items (`• **Short scan headline**: 2-4 sentences compact narrative synthesis`) with exact story partitioning per deterministic rubric block (no omissions, duplicates, or cross-block movements).
...
8. **Evidence-Bound Article Generation, Adaptive Profiles & Single-Call Budget** (`src/publication/article_context.py`, `src/publication/article_length.py`, `src/publication/article_models.py`, `src/publication/article_validator.py`, `src/publication/article_trace.py`, `src/article_generator.py`):
   - Packages deterministic `ArticleEditorialContext` preserving atomic `ArticleSupport` citable units, frozen reporting window bounds (`reporting_window_start`/`reporting_window_end` derived from frozen `lookback_hours`), and exact fragment texts without destroying provenance.
   - Computes adaptive length profiles (`thin` 300–700 words, `standard` 500–1100 words, `rich` 800–1400 words) with a hard floor of 180 words, avoiding artificial padding on thin news days.
   - Allows intuitive thematic section headings (`heading_claims: ()`) supported by paragraph evidence without requiring redundant claim atoms, while keeping concrete number/date/interval validation strictly enforced.
   - Enforces strict unit-level and claim-atom-level support citations (`TITLE`, `LEAD`, `P001`, ...) with `ArticleClaimAtom` across writer schema and prompt contracts.
   - Synthesizes long-form articles in a single LLM call into `StructuredArticleDraft` and persists an internal `ArticleClaimTrace` mapping discrete claim atoms to exact source items, fragments, and temporal roles.
   - Enforces strict $\le 1$ generative chat call budget per publication product: classifies outcomes honestly (`writer_success`, `validation_fallback`, `writer_error_fallback`, `deterministic_selected`) and activates deterministic fallback only when safety/validation fails.
...
- **Golden Regression Oracles**:
  ...
  pytest tests/test_edition_geography.py -v --no-cov
  ```
