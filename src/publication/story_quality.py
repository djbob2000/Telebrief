...
_NON_EDITORIAL_PAYLOAD_RE = re.compile(
    ...
    # Vague guesswork, search feedback, conversational chatter
    r"(?:наш[её]л\s+только\s+старые\s+объявления|что[- ]то\s+работает\s*\(вероятно|не\s+будет\s+ветреной\s+погоды|кому\s+надо\s+уже\s+проголосовали|я\s+не\s+ванга|выражает\s+пожелание,?\s+чтобы\s+город|эти\s+плохиши\s+пилили|плохиши\s+пилили)|"
