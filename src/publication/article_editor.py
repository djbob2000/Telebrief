...
class ArticleEditor:
    ...
    def _build_system_prompt(self) -> str:
        return (
            ...
            "8. ЧАТОВАЯ КУХНЯ И ИСТОЧНИКИ (CHAT_KITCHEN_LEAK):\n"
            "   - КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО использовать слова чатовой кухни («перекличка», «в перекличках», «в чате», «в каналах» и т.п.).\n"
        ...
        for u in unit_contexts:
            ...
            for iss in issues:
                ...
                if iss.code == "UNSUPPORTED_CLAIM_ATOM":
                    ...
                    msg += " -> ВАЖНО: полностью удалите слова чатовой кухни («перекличка», «в чате» и т.п.) и перепишите фразу через нормальную городскую атрибуцию («по сообщениям жителей», «ситуация обратная» и т.п.)!"
