...
_EQUIVALENCE_FAMILIES = (
    ...
    frozenset({"дзеркальн", "зеркальн"}),
    frozenset({"вывоз", "вивоз", "вывозят", "вивозять", "вивозит"}),
    frozenset(
        {"закрыт", "закрыва", "припиня", "припинен", "закрит", "закрива", "прекраща", "прекращен"}
    ),
    frozenset({"бывш", "колишн"}),
)
