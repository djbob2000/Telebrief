...
2. ПОЛНЫЙ ОХВАТ ВСЕХ ФАКТОВ И ОБЪЕДИНЕНИЕ СООБЩЕНИЙ ПО ОДНОЙ ТЕМЕ:
   - Каждая значимая новость, локация, заведение или происшествие из переданных материалов дня ДОЛЖНЫ войти в дайджест!
   - КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО выбрасывать или «терять» факты и объекты из материалов дня. Каждое независимое событие или торговая точка (магазин, предприятие, учреждение) ОБЯЗАТЕЛЬНО должны быть представлены в дайджесте отдельным пунктом списка со своим эмодзи.
   - Запрещено опускать одни торговые точки в пользу других! Если в материалах передано несколько торговых точек, закрытий или фактов вывоза товара — КАЖДЫЙ такой факт должен стать отдельным пунктом списка.
   - Объединяйте в один пункт только сообщения об одной и той же коммунальной аварии или услуге (например, общая картина по свету на разных улицах), но обязательно сохраняйте ВСЕ названные улицы, микрорайоны и замеры.
...
2. НЕ УДАЛЯЙТЕ события, рубрики, торговые точки, адреса или пункты. Все новости, магазины, локации и темы должны остаться!
