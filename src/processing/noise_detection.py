...
CLASSIFIED_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(
        r"(?:\b(цена|стоимость|руб|рублей|р\.|грн|usd|\$|₽)\b.*\b\d+\b|\b\d+\b.*\b(руб|рублей|р\.|грн|usd|\$|₽)\b)",
        re.IGNORECASE,
    ),
...
_DEDICATED_COMMERCIAL_SERVICE_PATTERN = re.compile(
    r"(?:🔴\s*вывоз мусора|вывоз (?:строй)?мусора.*(?:грузовые перевозки|грузоперевозки|хлам|строймусор)|"
    r"пассажирские перевозки.*(?:рейсы|микроавтобус|автобус|ежедневно)|"
    ...
    r"(?:рейс[ыа]|поездк[иа]|перевозк[иа]|пассажирские|маршрут|трансфер|выезд|автобус\w*).{0,60}(?:ростов|москв|крым|краснодар|таганрог|симферополь|донецк|мариуполь)|"
    r"(?:ростов|москв|крым|краснодар|таганрог|симферополь|донецк|мариуполь).{0,60}(?:рейс[ыа]|поездк[иа]|перевозк[иа]|пассажирские|маршрут|трансфер|выезд|автобус\w*)|"
    r"(?:busking|busking\.pro|забор\s+пассажиров\s+по\s+городским\s+остановкам|запись\s+вед[её]тся\s+через\s+(?:ресурс|сайт))|"
    r"(?:разблокировк|актуализаци|верификаци|восстановлени[ея]\s+доступа).{0,40}(?:карт|счетов|пенси|пф|приват|сбер|личных\s+кабинетов|онлайн-банкинг|а-банк|сенс|sense|райффайзен|укргазбанк)|"
    ...
    r"возьму попутчиков.*(?:выезд|тел|машин[еы]))",
    re.IGNORECASE,
)
...
_DIRECTORY_PAYLOAD_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(
        r"(?:^|\n)\s*(?:\*\*[^*]+\*\*|[🏛🏢🏪🏨🌟]\s*\S+).*(?:[🏛📍]?\s*(?:г\.\s*[\w-]+|ул\.|пр\.|адрес).*?[🕗🕒]?\s*(?:режим|график)\s+работы\s*:\s*(?:пн[.\s-]*сб|пн[.\s-]*пт|ежедневно|без выходных|\w+\s*[-—–]\s*выходной|с?\s*\d{1,2}[:.]\d{2})|[🕗🕒]?\s*(?:режим|график)\s+работы\s*:\s*(?:пн[.\s-]*сб|пн[.\s-]*пт|ежедневно|без выходных|\w+\s*[-—–]\s*выходной|с?\s*\d{1,2}[:.]\d{2}).*?[🏛📍]?\s*(?:г\.\s*[\w-]+|ул\.|пр\.|адрес))",
