...
4. СТРУКТУРА И ПЛАН ПОКРЫТИЯ (ARTICLE COVERAGE PLAN):
   ...
   - ПРИОРИТЕТЫ ПОКРЫТИЯ (DEVELOP, WEAVE, BRIEF):
     ...
     * СИНТЕЗ ОДНОРОДНЫХ СООБЩЕНИЙ (ГЕОГРАФИЯ И УЛИЦЫ):
       ...
       Синтезируйте их в ОДНО плавное, связное журналистское предложение с естественным географическим перечислением внутри соответствующего абзаца темы (например: «В то же время на улицах А, Б и В, а также в микрорайонах Г и Д жители сообщают о перебоях со светом...»).
       Такое предложение полностью раскрывает и покрывает все упомянутые сюжеты, сохраняя цельность и гладкость текста.
     * СОЦИАЛЬНЫЕ И ГОРОДСКИЕ МИКРОИСТОРИИ (СОХРАНЕНИЕ МИКРОДЕТАЛЕЙ):
       Небольшие социальные, бытовые и городские сюжеты органично вплетайте в соответствующий раздел хроники, сохраняя только подтверждённые сведения о том, кто, где и что сообщил или сделал.
       - СОХРАНЯЙТЕ ВСЕ КОНКРЕТНЫЕ МИКРОДЕТАЛИ (AGENTS.md 0.4): Если в фактах указан конкретный микрорайон, улица, ориентир , реакция скорой помощи или полиции — ОБЯЗАТЕЛЬНО сохраняйте эти конкретные названия и детали! КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО заменять конкретные топонимы из источников абстрактными клише вроде «в одном из районов города» или «в неназванном месте».
       - УВАЖИТЕЛЬНЫЙ ЯЗЫК: КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО копировать сообщения жителей как есть со знаками вопроса («Кто нибудь знает эту бабулю?...»). Всегда пересказывайте их уважительным языком в косвенной речи от третьего лица («В городе ищут родных пожилой женщины, которая с утра находится в районе N и не помнит свой адрес...»).
     * РАЗДЕЛЕНИЕ НА АБЗАЦЫ И ЗАПРЕТ НА ОДНОПРЕДЛОЖНЫЕ ОГРЫЗКИ (AGENTS.md 0.9):
       - КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО превращать статью в набор изолированных однопредложных абзацев («bullet-like paragraphs»)! Одиночные факты BRIEF (например, уровень напряжения в сети, отдельный магазин или расписание) ОБЯЗАНЫ вплетаться в связный абзац соответствующей темы, а не висеть отдельной строкой из одного предложения.
       - КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО сваливать разнородные темы в искусственный финальный абзац-«свалку» («В бытовом плане горожане продолжают...»). Каждая тема должна находиться в своём смысловом разделе.
5. ТОНАЛЬНОСТЬ, ЯЗЫК И АТРИБУЦИЯ:
   ...
   - Заголовок (title): емкий, информативный газетный заголовок дня, начинающийся с названия города или отражающий ключевое событие дня. Запрещено делать заголовком вопросы жителей.
...
3. НЕ РАСКРЫВАЙТЕ ИСТОЧНИКИ И ТЕХНИЧЕСКУЮ КУХНЮ СБОРА ДАННЫХ:
...
"""
    ...
    async def generate_from_event_article_context(  # noqa: C901
        ...
        user_prompt = (
            ...
            "5. ПРИЧИННО-СЛЕДСТВЕННЫЕ СВЯЗИ И СТИЛЬ: Строго соблюдайте Evidence Boundary — опирайтесь только на факты из материалов выше, не выдумывайте подробностей. Не домысливайте причин («из-за аварии», «вследствие пожара» допустимы только если причина прямо указана в источнике). Если причинно-следственная связь прямо не подтверждена, описывайте события рядом без слова «из-за». КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО использовать слова чатовой кухни и вскрывать источники («перекличка», «в чатах», «в каналах»); используйте естественную городскую атрибуцию («по сообщениям жителей», «ситуация обратная»).\n"
