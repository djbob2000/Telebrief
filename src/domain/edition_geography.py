...
_BERDYANSK_TOPONYM_RULES = (
    ...
    "«Зеркальный» («Дзеркальний») — бывший супермаркет и известный ориентир в Бердянске (НЕ кинотеатр!). В русскоязычном тексте писать: «бывший супермаркет «Зеркальный»» или «возле «Зеркального»».",
)
...
def normalize_edition_toponyms(text: str, edition_slug: str = "berdyansk") -> str:
    ...
    # 10. «Зеркальный» (бывший супермаркет, не кинотеатр)
    res = re.sub(
        r"(?:\b(?:возле|рядом\s+с|у)\s+)?кинотеатр[а-я]*\s+[«\"]?(?:Д)?зеркальн[а-я]*[»\"]?",
        "возле бывшего супермаркета «Зеркальный»",
        res,
        flags=re.IGNORECASE,
    )
    res = re.sub(
