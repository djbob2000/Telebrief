...
def test_digest_quality_floor_on_rendered_prose():
    ...
    sample_digest = """# Дайджест · 02 сентября 2026

Коммунальная обстановка

⚡ **Электроснабжение.** По сообщениям жителей, свет отключили в нагорной части города и на Слободке; в центре зафиксировано низкое напряжение около 170 В, а на улице Петровского электроснабжение восстановили.

💧 **Водоснабжение.** В центральных районах подача воды осуществляется по временному графику с 17:00 до 21:00, организован подвоз технической воды к школе №3.

Транспорт и связь

🚌 **Городской транспорт.** Автобусы курсируют по штатному расписанию, на всех маршрутах действует безналичная оплата через терминалы.
"""
    assert "Городская обстановка" not in sample_digest
    assert "🔴" not in sample_digest
    assert "🟡" not in sample_digest
    assert "🟢" not in sample_digest
    assert sample_digest.count("Электроснабжение") == 1
